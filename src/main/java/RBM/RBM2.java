...
public class RBM2 {
   ...
   public static Log log = new Log(RBM2.class);
   ...
   RBMupdater2 updater; // collects the training data and updates the parameters
    ...
    */
   public RBM2(int num_visible_nodes, int num_hidden_nodes) {
   ...
   public RBMupdater2 getUpdater() {
      return new RBMupdater2( this );   
   }
    ...
    */
   public void train(RBMtrainingset set, double learning_rate, double momentum, int max_epochs) {
      /**
      ...
      for (int epoch = 0; epoch < max_epochs; epoch++) {
         ...
         for (Sample sample : set.randomIterable()) {
            contrastive_divergence(sample, trainedparams );
            updater.store_weight_updates(sample);
            updater.update_weights(trainedparams, learning_rate, momentum);
         }
         updater.update_weights(trainedparams, learning_rate, momentum);

         // evaluate the quality of the generated model
         compute_sse(set, trainedparams);
         if (trainedparams.sse >= params.sse && RandomTools.getBoolean()) {
            log.printf("aap");
            for (Sample sample : set.randomIterable()) {
               awake_divergence(sample, trainedparams );
               updater.store_weight_updates(sample);
            }
            updater.update_weights(trainedparams, learning_rate, momentum);
            compute_sse(set, trainedparams);
         }
         log.info("%f %f", trainedparams.sse, params.sse);
   ...
   public void contrastive_divergence(Sample sample, RBMparameterset params) {
      estimate_hidden(sample.visible, params, p_h1, state_h1);
      estimate_visible(state_h1, params, p_v2);
      estimate_hidden(p_v2, params, p_h2, state_h2);
   }

   public void awake_divergence(Sample sample, RBMparameterset params ) {
   ...
   public void estimate_visible_likelihood(Matrix visible, RBMparameterset params, Matrix p_v2) {
      ...
      double act_p[] = p_h1.getRow(0);                        // to store the activation probability
      double state_hidden[] = state_h1.getRow(0);             // to store the states of the hidden layer
      double reconstructed_visible[] = p_v2.getRow(0);        // to store the reconstructed visible nodes
      ArrayTools.fill(p_v2.value[0], 0);                      // set the value of the reconstructed nodes to 0
      double v1[] = visible.getRow(0);                        // contains the values for the visible input nodes

      // try all combinations of 2^num_v binary visible nodes
      // if a i-th bit is set in possiblestate, this represent the hidden node i is true
      for (int possiblestate = (1 << num_h) - 1; possiblestate >= 0; possiblestate--) {

         // compute the likelihood that this state occurs
         double statelikelihood = 1;
         for (int j = 0; j < num_h; j++) {
            state_hidden[j] = ((possiblestate & (1 << j)) > 0) ? 1 : 0;
            statelikelihood *= (state_hidden[j] == 1) ? act_p[j] : (1 - act_p[j]);
         }

         // estimate visible outcome of this state
         // todo: precompute the possible outcomes once, to improve speed
         estimate_visible(state_h1, params, p_vtemp);

         // interpolate mle of reconstructed visible nodes using statelikelihood and stateresults 
         for (int node = 0; node < num_v; node++) {
            reconstructed_visible[node] += statelikelihood * p_vtemp.value[0][node];
         }
      }
   }

   // compute sum of squared errors between visible input and reconstructed visible output for a model
   public void compute_sse(RBMtrainingset set, RBMparameterset params) {
      params.sse = 0;
      double v2[] = p_v2.getRow(0);
      for (Sample sample : set) {
         double v1[] = sample.visible.getRow(0);
         estimate_visible_likelihood(sample.visible, params, p_v2);
         for (int node = 0; node < num_v; node++) {
            if (!sample.missing[node]) {
               params.sse += Math.pow(v1[ node] - v2[ node], 2);
            }
         }
      }
   }
